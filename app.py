...
@app.route('/api/<game_code>/guess/<word_id>')
def make_guess(game_code, word_id):
    game = get_game_by_id(game_code)
    if game:
        if hasattr(game, f'word{word_id}status'):
            setattr(game, f'word{word_id}status', 1)
            db.session.commit()
            return {"message": "Status updated successfully"}, 200
        else:
            return {"error": "Invalid word number"}, 400
    else:
        return {"error": "Game not found"}, 404
